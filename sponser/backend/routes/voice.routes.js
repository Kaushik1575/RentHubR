...
// Helper to send email if emailService exists
let emailService;
try {
    emailService = require('../../user/backend/config/emailService');
} catch (e) {
    try {
        ...
        try {
            if (bookingId && supabase) {
                ...
                if (error) {
                    ...
                    if (userEmail && emailService?.sendBookingConfirmationEmail) {
                        await emailService.sendBookingConfirmationEmail(userEmail, userName, booking);
                    }
                }
            }
        } catch (dbErr) {
        ...
        try {
            if (bookingId && supabase) {
                ...
                if (error) {
                    ...
                    if (userEmail && emailService?.sendBookingCancelledEmail) {
                        await emailService.sendBookingCancelledEmail(userEmail, userName, bookingId, vehicleName);
    ...
    try {
        ...
        if (action === 'confirm' || action === 'confirm_booking' || event === 'booking_confirmed') {
            ...
            if (bookingId && supabase) {
                ...
                if (email && emailService?.sendBookingConfirmationEmail) {
                    await emailService.sendBookingConfirmationEmail(email, userName, booking);
                }
            }
            ...
            if (bookingId && supabase) {
                ...
                const booking = (data && data[0]) || {};
                const email = userEmail || booking.users?.email || booking.user_email;
                if (email && emailService?.sendBookingCancelledEmail) {
                    await emailService.sendBookingCancelledEmail(email, userName, bookingId, vehicleName);
                }
            }
