    ...
    try {
        ...
        if (status === 'send_nearby_locations' || status === 'send_location' || status === 'send_nearest_locations') {
            ...
            if (userEmail) {
                ...
                console.log(`✉️ [Admin SOS] Dispatched nearest locations email to ${userEmail} for booking ${bookingId}`);
            }
...
const sendNearestLocations = async (req, res) => {
    try {
        ...
        console.log(`📍 [Admin SOS Controller] Finding nearest locations for Booking: ${bookingId}, Email: ${userEmail}`);
